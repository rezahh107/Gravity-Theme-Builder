...
check( GTB_SRWF_RUNTIME_DIAGNOSTIC_VERSION === '0.3.1', 'unexpected diagnostic package version' );
...
check( $scripts[1]['handle'] === 'gtb-srwf-admission-diagnostic-v031', 'wrong admission diagnostic handle' );
check( $scripts[1]['ver'] === '0.3.1' && $scripts[1]['footer'] === true, 'wrong admission diagnostic metadata' );
check( count( $inline_scripts ) === 1, 'normal target missing bounded admission payload' );
check( $inline_scripts[0]['handle'] === 'gtb-srwf-admission-diagnostic-v031' && $inline_scripts[0]['position'] === 'before', 'admission payload attached to wrong script' );
...
echo "PASS: diagnostic delivery and context observability contract\n";
