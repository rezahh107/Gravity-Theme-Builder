        ...
        if char == ":":
            ...
            if index < len(selector) and selector[index] == "(":
                ...
                if name in {"is", "not", "has"}:
                    specs = [selector_specificity(argument) for argument in split_selector_list(arguments)]
                    arg_ids, arg_classes, arg_types = max(specs, default=(0, 0, 0))
                    ids += arg_ids
                    classes += arg_classes
                    types += arg_types
                elif name != "where":
...
class SrwfRegistrationStaticTests(unittest.TestCase):
    ...
    def test_css_uses_only_reviewed_gravity_forms_api_identifiers(self) -> None:
        used = set(re.findall(r"--gf-[a-z0-9-]+", CSS.read_text(encoding="utf-8")))
    ...
    def test_token_enforcement_specificity_outranks_orbital_per_form_scope(self) -> None:
        ...
        self.assertGreater(enforcement_specificity, host_specificity)

    def test_token_enforcement_boundary_is_documented_theme_framework_scope(self) -> None:
        token_blocks = gf_token_blocks(CSS.read_text(encoding="utf-8"))
    ...
    def test_every_reviewed_gf_token_is_under_the_repaired_boundary(self) -> None:
        token_blocks = gf_token_blocks(CSS.read_text(encoding="utf-8"))
        self.assertEqual(1, len(token_blocks))
        self.assertEqual(REVIEWED_GF_API, {name for name, _ in gf_token_declarations(token_blocks[0][1])})

    def test_gf_token_values_have_single_implementation_source(self) -> None:
        declarations = [
            name
            for _, body in css_blocks(CSS.read_text(encoding="utf-8"))
            for name, _ in gf_token_declarations(body)
        ]
        counts = Counter(declarations)
        self.assertEqual(REVIEWED_GF_API, set(counts))
        self.assertTrue(all(count == 1 for count in counts.values()), f"duplicate --gf-* declarations found: {counts}")

    def test_non_token_selectors_remain_under_the_bounded_opt_in_scope(self) -> None:
        for selector_group, body in css_blocks(CSS.read_text(encoding="utf-8")):
    ...
    def test_unresolved_values_are_not_promoted(self) -> None:
        ...
        title = re.search(r"\.gform-theme--framework\.srwf-registration-theme_wrapper \.gform_title\s*\{([^}]*)\}", css, re.S)
