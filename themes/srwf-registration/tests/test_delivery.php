...
check( SRWF_REGISTRATION_THEME_VERSION === '0.1.6', 'unexpected SRWF test package version' );
...
check( $styles[0]['ver'] === '0.1.6', 'stylesheet package version not propagated' );
...
check( array_filter( $hooks, fn( $h ) => $h[0] === 'filter' && $h[1] === 'gform_form_settings_menu' && $h[2] === 'srwf_registration_gtb_settings_menu' ) !== array(), 'native GTB Form Settings menu hook missing' );
check( array_filter( $hooks, fn( $h ) => $h[0] === 'action' && $h[1] === 'gform_form_settings_page_gtb_theme' && $h[2] === 'srwf_registration_gtb_settings_page' ) !== array(), 'native GTB Form Settings page hook missing' );
