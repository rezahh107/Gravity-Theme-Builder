...
function srwf_registration_gtb_recommended_draft( $form, $config ) {
    ...
    if ( empty( $config['enabled'] ) ) {
        $changes[] = 'Proposal: enable the SRWF Registration presentation for this form.';
    }
    foreach ( srwf_registration_gtb_role_definitions() as $key => $definition ) {
        ...
        if ( 1 === count( $matches ) ) {
            ...
            $changes[] = sprintf( 'Proposal: map %s to field #%d because that field already carries the explicit GTB token %s.', $definition['label'], $matches[0], $definition['token'] );
    ...
    return array( 'config' => $draft, 'notes' => $notes, 'changes' => $changes );
}
...
function srwf_registration_gtb_settings_page() {
    ...
    if ( 'save' === $action ) {
        ...
        $proposal_changes = $recommendation['changes'];
    }
    ...
    foreach ( $proposal_changes as $change ) {
        srwf_registration_gtb_notice( 'info', $change );
    }
    ...
    echo '<button type="submit" class="button" name="gtb_action" value="recommend">' . esc_html__( 'Load Recommended SRWF Draft', 'gravity-theme-builder' ) . '</button> ';
