...
- GP File Upload Pro — initial state only: shared SRWF upload-family CSS stops at `.gpfup--has-files`. Report Card specialization is explicit through `srwf-role-report-card-upload` and its local document glyph; authentic `gpfup--images-only` Student Photo uses the same decorative icon-slot geometry with its own GTB-owned photo/camera glyph. Upload/progress/validation/preview/delete/crop/storage remain GPFUP-owned; the theme does not add replacement upload markup or JavaScript lifecycle.
