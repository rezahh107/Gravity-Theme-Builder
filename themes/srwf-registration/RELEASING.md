...
SRWF Registration releases are created deliberately from a specific commit that is already merged into `main`. The release workflow never bumps the version, never republishes an existing release identity, and never publishes the diagnostic package.
...
Immediately before mutation, the publish job rechecks the exact source identity and `main` ancestry, verifies the candidate checksum, and rechecks that neither the intended tag nor Release appeared since the dry-run stage. Existing tag/release identities are never reused or edited by this workflow.
