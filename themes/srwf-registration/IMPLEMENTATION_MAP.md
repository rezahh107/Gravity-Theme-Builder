...
| Report Card initial GPFUP | min `96px`; pad `16px`; radius `12px`; dashed `#8690A1` | explicit Report Card role + authentic `.gpfup:not(.gpfup--has-files)` | `OWNER_AUTHORIZED / STATICALLY_IMPLEMENTED`; upload runtime pending |
...
| Student Photo post-upload | use authentic Photo consumer only | no speculative selectors/composition | `OWNER_RUNTIME_REQUIRED / NOT_PROVEN`; crop ratio/dimensions `HOST_OWNED` |
