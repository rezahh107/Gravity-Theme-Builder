...
Status: **FIRST_IMPLEMENTATION / VISUAL_UX_V1.0.1_REGISTERED / PER_FORM_GTB_CONFIGURATION_RUNTIME_PROVEN / RADIO_VISUAL_RECHECK_REQUIRED**
...
|---|---|---|---|---|---|---|---|---|---|
| Theme activation boundary | Explicit opt-in; unrelated forms must remain untouched | Charter §9; Theme Authoring Contract §7 | isolation / activation | native per-form **Settings → GTB Theme** stores SRWF enablement and explicitly projects `srwf-registration-theme`; runtime continues to use `gform_form_theme_slug` + `gform_enqueue_scripts` | per-form settings hooks + `GFAPI::update_form()` `DOCUMENTED`; Owner 2026-09-18 Save/projection evidence `RUNTIME_PROVEN` | `RUNTIME_PROVEN` for captured form | regression recheck after future host changes | fail closed when activation token is absent | `src/srwf-registration-settings.php`; `src/srwf-registration-theme.php`; `.srwf-registration-theme_wrapper` |
| Theme Framework base | Use Gravity Forms Theme Framework; Orbital is current implementation | Canonical GF reference §§1–3 | host integration | force `orbital` only for opted-in/admitted form | `DOCUMENTED`; Owner Registration runtime shows Framework/Orbital + SRWF stylesheet | `RUNTIME_PROVEN` for captured Registration render | regression recheck | no legacy fallback | `src/srwf-registration-theme.php` |
| Persian composition | RTL | `VA:VC-1.0.1` §10 | directionality | theme-scoped CSS `direction: rtl` | `OWNER_APPROVED`; CSS mechanism `DOCUMENTED` | `IMPLEMENT` | yes — full visual/reading-order qualification remains open | preserve host semantics | root scope in `srwf-registration.css` |
...
| Primary action | full-width, blue, 56px minimum, `16px / 700` | `VA:VC-1.0.1` §§8,9,13 + `VA:ARTIFACT` | action / control | button CSS API + real `<button.gform_button>` + sentinel-scoped width repair | Submit conflict and final repair `RUNTIME_PROVEN` by Owner after PR #9; later diagnostic still observed `840×56` | `RUNTIME_PROVEN` | regression recheck | no IDs/JS/`!important` | button properties + sentinel footer rule |
| School selector / GP Advanced Select | ordinary-control visual family, minimum ~52px | `VA:VC-1.0.1` §§13,18,22 | add-on integration | runtime-proven `.ts-wrapper > .ts-control`; scoped min-size adapter | repaired consumer `RUNTIME_PROVEN` by Owner after PR #9 | `RUNTIME_PROVEN` | regression recheck | no behavior JS | `.ts-wrapper .ts-control` |
| Gender binary card role | two equal-width approved card choices; native radio owns state | `VA:VC-1.0.1` §27 `gender_binary_choice` + `VA:ARTIFACT` | choice presentation | **GTB Theme** Gender mapping projects `srwf-role-binary-choice`; CSS explicitly overrides host vertical choice direction with an intrinsic row and equal authentic `.gchoice` tracks | mapping/projection `RUNTIME_PROVEN`; 0.1.6 Owner visual recheck found geometry only partial; 0.1.7 repair `STATICALLY_PROVEN` | `OWNER_RUNTIME_RECHECK_REQUIRED` | yes — recheck exact 0.1.7 card geometry/click/keyboard | native ordinary radio if role unresolved | `src/srwf-registration-settings.php`; `.srwf-role-binary-choice` rules; `evidence/OWNER_VISUAL_RECHECK_2026-09-18.md` |
| Graduation Status binary card role | same approved two-card family when GF reveals field | `VA:VC-1.0.1` §27 `graduation_status_binary_choice` + `VA:ARTIFACT` | conditional choice presentation | separate **GTB Theme** Graduation mapping projects same role; CSS binary row is shared but GF retains conditional visibility | mapping/projection `RUNTIME_PROVEN`; 0.1.7 visual repair `STATICALLY_PROVEN` | `OWNER_RUNTIME_RECHECK_REQUIRED` | yes — exercise authentic conditional reveal and geometry | GF conditional logic + ordinary radio fallback | `src/srwf-registration-settings.php`; `.srwf-role-binary-choice` rules; visual recheck evidence |
| Ordinary radio choice presentation | vertical full-width choice cards; 10px stack gap; 48px minimum; 20px authentic radio with 10px check | `VA:VC-1.0.1` §13 + `VA:ARTIFACT` Education Level `vertical-choices` family | choice presentation | scoped `.gfield--type-radio:not(.srwf-role-binary-choice)` styles authentic `.gfield_radio`/`.gchoice`; documented `--gf-ctrl-choice-size` and `--gf-ctrl-radio-check-size`; native radio remains checked/focus/keyboard authority | admitted artifact re-materialized; GF API identifiers `DOCUMENTED`; 0.1.7 tests `STATICALLY_PROVEN`; Owner reported 0.1.6 ordinary radios incorrect | `OWNER_RUNTIME_RECHECK_REQUIRED` | yes — recheck Education Level and other ordinary radio groups on real form | native GF radio when SRWF theme absent | `src/srwf-registration.css`; `tests/test_radio_choice_presentation.py`; visual recheck evidence |
| Section iconography | exact admitted icon geometry only on explicitly mapped Section Break roles | `VA:VC-1.0.1` §27 `section_heading_iconography` + `VA:ARTIFACT` | decorative section presentation | five separate **GTB Theme** Section Break mappings project one exact semantic token each + local SVG assets | mapping/token projection runtime-proven; SVG geometry tests `STATICALLY_PROVEN` | `IMPLEMENTED / VISUAL_RECHECK_OPEN` | yes — final visual parity still open | no icon when role unresolved | `src/srwf-registration-settings.php`; `src/icons/section-*.svg`; mapped selectors |
| Report Card initial GPFUP | compact dashed initial surface, exact file icon, persistent host rules | `VA:VC-1.0.1` §27 `report_card_upload_initial` + `VA:ARTIFACT` | add-on presentation | **GTB Theme** Report Card mapping projects `srwf-role-report-card-upload`; authentic `.gpfup` consumer remains host-owned | GPFUP consumer `RUNTIME_PROVEN`; projection runtime-proven; presentation static | `IMPLEMENTED / VISUAL_RECHECK_OPEN` | yes — inspect empty state | host-native GPFUP when role unresolved | `src/srwf-registration-settings.php`; `.srwf-role-report-card-upload` rules + `report-card-file.svg` |
...
| Form max width | `840px` | `VA:VC-1.0.1` §§8,11,17 | layout | intrinsic `max-inline-size` on activation wrapper | CSS mechanism stable; Owner runtime observed 840px wrapper | `RUNTIME_OBSERVED` | responsive regression recheck | none | root scope |
| Mobile horizontal padding | `16px` | `VA:VC-1.0.1` §§8,11 | responsive / shell | needs resolved host/breakpoint ownership | value `OWNER_APPROVED`; production breakpoint `NOT_PROVEN`; Owner explicitly has not tested responsive in current pass | `DEFER_NOT_PROVEN` | yes | host shell may temporarily own | none |
...
| Production breakpoint | `NOT_PROVEN` | `VA:VC-1.0.1` §§11,22,27 | responsive | none authorized | `NOT_PROVEN`; responsive Owner test has not occurred | `DEFER_NOT_PROVEN` | yes | intrinsic layout only | no `@media` |
...
`READY` is configuration/projection readiness. It is **not** a claim of complete visual or responsive qualification.
...
The 2026-09-18 per-form configuration pass then proved on the Owner runtime that **Settings → GTB Theme** loads, explicit Save persists, all eight semantic mappings project their owned tokens, configuration reports `READY`, and Registration admission/enqueue succeeds. See `evidence/OWNER_RUNTIME_GTB_CONFIGURATION_2026-09-18.md`.

The subsequent Owner visual recheck explicitly found that 0.1.6 did **not** finish visual radio qualification: Gender was only partially correct and incorrectly sized/oriented, ordinary radio menus were not correct, and responsive behavior had not been tested. Exact artifact re-inspection identified the missing distinction between binary `choice-row` cards and ordinary `vertical-choices` cards; 0.1.7 implements that repair and now requires Owner runtime recheck. See `evidence/OWNER_VISUAL_RECHECK_2026-09-18.md`.

The runtime evidence still does **not** resolve or qualify the production breakpoint, 320/360/390/430 responsive acceptance, desktop short-field pairing target, desktop shadow, title metrics, field/section rhythm, exact focus-ring geometry/alpha, helper/error exact sizing, page-shell ownership, PersianGravity picker presentation, Student Photo post-upload/crop DOM, or complete validation/text-enlargement behavior. Those remain explicitly unresolved under `VA:VC-1.0.1`.
...
4. Use **Check Again** to verify configuration/projection `READY`; it is read-only and cannot repair configuration. `READY` does not certify visual/responsive parity.
...
9. The 0.1.7 radio repair must be Owner-rechecked on the authentic form before binary/ordinary radio presentation is promoted to `RUNTIME_PROVEN`; responsive acceptance remains separately open.
